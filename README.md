...
If they want, the user can also add a button to the title scene that, when pressed, invokes the "ChangeLocal.change_local(string s)" function, where the parameters represents the locale code of the language added. 
### Localization
We localized the game into Brazilian Portuguese, Hebrew (right-to-left), and Simplified Chinese (logographic). These languages were chosen because team members are native speakers or fluent in them (Ethan speaks Hebrew, Igor speaks portugese, and Eric speaks chinese). We set up a google sheet with one english key in each row, and everyone filled out their corresponding translations on their own time. 
...
## Reflection
Ethan was invaluable for this part of the assignment, and it would have been genuninely impossible for us to get the mobile port working without him. As discussed in the mobile installation, Godot only has andriod support, and Ethan is the only group member with an android phone. As such, he was kind of the only choice for who would be able to port the game to moblie. This could have been a nightmare for him, but he also just so happened to it set up for testing games on mobile, so he didn't really have to do too much work to get the port on his phone. Without him, this requirement would have been a nightmare, but thanks to his help, this was probably the easiest portion of whole project.  
Similarly, the localization was a breeze as well. Godot is well set up to deal with localization, so the programming part took Moore all of two hours. It took signifigantly more time to replace the text in the UI with icons and add the new flag buttons than it was to get Godot to read in the csv file and apply the translations. We also had enough people who spoke other languages that we could just ask them to translate the text, so we didn't have to worry about getting other people outside the class involved.  
