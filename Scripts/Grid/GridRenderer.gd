...
func init(gd: Node, op : Resource) -> void:
    ...
    grid_manager = gd
    grid_sprites = []
    for i in range(options.get("gridDimensions")):
        grid_sprites.append([])
        for j in range(options.get("gridDimensions")):
...
func render_cell(x : int, y : int) -> void:
    var cell = grid_manager.fetchCellArray(x, y)
    if grid_sprites[x][y] == null:
        ...
        cell_node.position = Vector2(
            (x - options.get("gridDimensions") / 2) * cell_sprite.texture.get_width() * cell_sprite.scale.x,
            (y - options.get("gridDimensions") / 2) * cell_sprite.texture.get_width() * cell_sprite.scale.y
        )
        grid_sprites[x][y] = cell_node
        grid_manager.add_child(grid_sprites[x][y])
    grid_sprites[x][y].call("update_labels", cell)
