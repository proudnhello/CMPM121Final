...
func _ready() -> void:
	...
	gridRenderer.call("init", self, options);

	seed(actionTracker.call("get_seed"));
	baseSeed = actionTracker.call("get_seed");

	# Set the player's movement distance		
	player.call("init", gridRenderer.call("get_cell_size"),
						options.get("gridDimensions")-1, 
							   Vector2(floor(options.get("gridDimensions")/2),
										   floor((options.get("gridDimensions")/2))));
...
func _can_plant(x, y, plantType) -> bool:
	if(grid._fetch_cell(x, y)[2] != 0 || inventory.call("GetItemAmnt", plantType - 1) <= 0):
		return false;
	else:
		return true;
...
func _try_harvest_plant(x, y):
	...
	actionTracker.call("harvest_plant", x, y, grid._fetch_cell(x, y)[2]);
	_harvest_plant(x, y);

func _progress_time_button() -> void:
	var timeStepSeed = actionTracker.call("get_next_seed");
...
func _unstep_time(actionInfo: Array):
	...
	if(sunSeed < actionTracker.call("get_seed")):
		sunSeed = 0;
...
func _undo_action_button():
	var actionInfo = actionTracker.call("undo_action");
...
func _redo_action_button():
	var actionInfo = actionTracker.call("redo_action");
	if(actionInfo == null || actionInfo.size() == 0): return;
...
func _load_file(saveName):
	...
	var actions = actionTracker.call("load", saveName);
	var actionArray = [];
